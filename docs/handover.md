...
| 验证     | `pnpm build / test / typecheck / lint` 全绿（**411 例测试**：protocol 26 / inspector 138 / bridge 74 / extension 173）；真机 `.playwright-mcp/verify-codex-card-ui.mjs` **106/106 断言全过**                                              |
...
UI Tuner/
  ...
  packages/
    ...
    inspector/                 chrome-free DOM 能力：Picker / Overlay / Selection /
                               styles(白名单/解析/取色/alpha 提取) / PreviewEngine / ChangeTracker /
...
| **`ColorRow` 色板旁加独立 alpha 滑块（`<input type="range">`）**，提交时 `formatColorWithAlpha(hex, alpha)` 组合输出：alpha≥1 → `#rrggbb`，0<alpha<1 → `rgba(r,g,b,a)`，alpha=0 → `transparent`；`extractAlpha(raw)` 从任意 CSS 颜色格式（rgb/rgba/hex/hex8/transparent）提取 alpha | `<input type="color">` 只支持不透明 `#rrggbb`，无法表达 alpha。此前提交一律写 opaque hex，半透明颜色丢失透明度。比较层（`colorKey`/`cssValuesEqual`）已是 alpha-aware，缺口在提交路径。`RGB_LEGACY` 正则的 `[,\s.]*` 字符类会吃掉 `.25` 的小数点（已修） |
...
6. **间距组件对齐 Codex 截图**：`StylePanel.tsx` 的 `SpacingGroup` 重写为 Codex 风格折叠/展开。折叠态：4 个带 `border border-edge bg-inset` 的值药丸（top/right/bottom/left）+ `ChevronRightIcon`。展开态：4 个 `ScrubField` 行，topbottom 和 left↔right 之间插 `LinkIcon`（`links-line`，新增到 `icons.tsx`）。`SizeGroup`（width/height）同理：两行之间插 `LinkIcon`。删除旧 `AxisScrub` / Simple+Advanced 双模式。
7. **头部布局对齐 Codex**：用户截图显示标签名与拖拽把手应在同一行。`EditorCard.tsx` 展开态头部改成两行结构：第一行 `⚙ + badge + instruction input`，第二行 `tagName + ml-auto spacer + drag handle`。拖拽把手始终渲染（即使无 tagName），保证测试与交互一致。
8. **锁定按钮可点击切换**：用户反馈「上下、左右、宽高那个是个锁定比例按钮，点击可以不锁定」。`SizeGroup` 和 `SpacingGroup` 的 `LinkIcon` 从装饰性 `<div>` 改成 `<button>`，点击调 `toggleLinked(pairKey)`。`StyleEditApi` 新增 `linked: ReadonlySet<string>` 和 `toggleLinked(pairKey: string): void`。`EditorCard` 维护 `linkedSet` 状态（默认 `width-height`/`top-bottom`/`left-right` 全锁定），`toggleLinked` 切换集合成员。锁定态图标 `text-accent-text`，未锁定 `text-faint hover:text-text`。`padding`/`margin` 展开态的 top↔bottom 和 left↔right 各一个锁定按钮（删除了原来多余的第三个 link icon）。i18n 新增 `style.lock`（锁定比例）/ `style.unlock`（解除锁定）。
9. **属性行高度加大**：用户反馈「单个属性的空间框高度大一些，目前太窄了」。`Row` 组件 `min-h-6 py-0.5` → `min-h-8 py-1`。所有控件高度 `h-6` → `h-7`：`ScrubInput`（拖动态 + 编辑态）、`TextRow`、`ColorRow`、`SelectRow`。
...
**颜色 alpha 保留（2026-09-07）**

Backlog 项「色板提交写 `#rrggbb` 丢失半透明 alpha」。

- 根因：`<input type="color">` 只能产出不透明 `#rrggbb`，`ColorRow` 提交路径只写 hex。比较层（`colorKey`/`cssValuesEqual`）已是 alpha-aware，缺口在提交。
- 实现：`inspector/styles/color.ts` 新增 `extractAlpha(raw)`（从 rgb/rgba/hex/hex8/transparent 提取 alpha 0–1）+ `formatColorWithAlpha(hex, alpha)`（alpha≥1 → hex，0<alpha<1 → `rgba(r,g,b,a)`，alpha=0 → `transparent`）。`ColorRow` 色板旁加 `<input type="range" min=0 max=1 step=0.01>` 透明度滑块，本地 `draftAlpha` 跟踪拖动期值，`onPointerUp`/`onBlur` 提交 `formatColorWithAlpha(hex, alpha)`。
- 附带修复：`RGB_LEGACY` 正则 `[,\s.]*` 字符类吃掉 `.25` 的小数点（`Number("25")` = 25 > 1 → alpha 被截为 1），改 `[,\s]*`。
- `EditorCard.test.tsx` 修 `getByRole("slider")` → `getAllByRole("slider").find(el => el.tagName === "DIV")`（alpha range input 引入额外 slider 角色元素）。
- 验证：`pnpm build/test/typecheck/lint` 全绿，**411 例**（inspector 119 → **138**：extractAlpha 14 + formatColorWithAlpha 5 + ColorRow alpha 4 + rows 既有；extension 169 → **173**：EditorCard slider 定位修复 + ColorRow alpha 相关）。
...
**注意**：顺延项都在 `docs/backlog.md`（Next App Router 适配、数据驱动文本索引、索引缓存、HMR 跨刷新持久化 §37、CLI npm 发布、codex MCP 免 bypass flag、ui_capture 元素级裁剪、**编辑卡语音输入**（麦克风目前是禁用占位））。
