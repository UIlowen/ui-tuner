...
7. **头部布局对齐 Codex**：用户截图显示拖拽把手应跨指令行与标签名行。`EditorCard.tsx` 展开态头部从 `<header>` 单行改成外层 `div.flex` 包裹：左侧 `flex-col` 放指令行（⚙ + badge + input）与标签名行，右侧拖拽把手 `span` 跨两行高度。标签名从独立 `border-b` 块改成指令行下方的 `span`（`mt-0.5`），不再占一整行高度。
