...
| **`ColorRow` 色板 + R/G/B/A 数字输入框**（替透明度滑块）：`<input type="color">` 色板做可视化选色，四个 `<input type="number">` 分别控制 R(0-255) / G(0-255) / B(0-255) / A(0-1)；每个输入框 `onChange` → preview，`onBlur` → commit；超范围值 clamp 后提交。`hexToRgb(hex)` 把色板 hex 拆成通道，`formatColorWithAlpha` 组合输出：alpha≥1 → `#rrggbb`，0<alpha<1 → `rgba(r,g,b,a)`，alpha=0 → `transparent`；`extractAlpha(raw)` 从任意 CSS 颜色格式提取 alpha | `<input type="color">` 只支持不透明 `#rrggbb`，无法表达 alpha。数字输入框比滑块更精确（原生取色器风格），且与 R/G/B 通道统一交互模式。比较层（`colorKey`/`cssValuesEqual`）已是 alpha-aware，缺口在提交路径 |
...
**颜色 RGBA 输入（2026-09-07）**

Backlog 项「色板提交写 `#rrggbb` 丢失半透明 alpha」→ 先用透明度滑块解决 → 用户要求改为 R/G/B/A 数字输入框（原生取色器风格）。

- 根因：`<input type="color">` 只能产出不透明 `#rrggbb`，无法表达 alpha。
- 实现：`inspector/styles/color.ts` 新增 `hexToRgb(hex)` 把色板 hex 拆成 R/G/B 通道。`ColorRow` 改为色板 + 四个 `<input type="number">`（R 0-255 / G 0-255 / B 0-255 / A 0-1），每个输入框 `onChange` → preview，`onBlur` → commit；超范围值 clamp 后提交。`formatColorWithAlpha(hex, alpha)` 组合输出：alpha≥1 → hex，0<alpha<1 → `rgba(r,g,b,a)`，alpha=0 → `transparent`。
