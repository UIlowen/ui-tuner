...
2. 面板 Style Tab 各行读 `store.styleValues`（提交值快照）。ScrubInput 拖动帧 → `updateStyle(prop, "20px", false)` → `sidepanel.stylePreview {committed:false}` —— **不更新 store 的 `styleValues`**。注意「零重渲染」只是理想假设：预览帧虽不改 `styleValues`，但会更新 `log`，且每帧 content 回的 `preview.changed` 更新 `changes`，而 App 顶层订阅了 `log` 与 `changes`（徽标）→ **整个面板仍会重渲染**。因此**任何绑定滞后 `styleValues` 的受控控件都会被重渲染拨回原值**：① `ScrubInput` 原来每次渲染都执行 `currentValue.current = value`，拖动中被滞后 prop 重置 → pointerUp 提交原值 → 页面回退；修复为 `useEffect` 仅在「非拖动且非编辑」时同步 ref。② `ColorRow` 原生取色框同理被拨回原色；修复为本地 `draft` state 跟踪拖动期实时色值，blur 提交 draft。两者是同一 bug 类（受控值滞后 + 父级重渲染回拨）。
...
- `chrome-extension`（50 例）：Channel 内存端口对（投递/丢弃/退订/断连）；store 状态机（连接、RTT、picking ack、selection+styleValues 路由、updateStyle 预览帧不改 store/提交更新/null 删除、preview.changed 镜像、elementNames 累积、revert/reset 动作出站消息、Bridge 握手 hello/welcome、selection/changes 自动 sync、断线 offline、sourceResolved 落库 + stale 守卫 + 重选/清除/断线置 null、M7：bridge.agents 落库/断线清空、agent.request 发送+sent 状态、离线 no-op、agent.applied 横幅/dismiss、agent.capture 往返含注入截图/无截图降级、**M8：applyChanges(scope) 只发选中元素 changes、apply.result 落库 + stale 守卫 + 成功发 sidepanel.confirmApply、apply.confirmed 计数、clearApplyState**、日志截断、reset）；**控件组件回归测试（jsdom + RTL，新增扩展组件测试基建）：`ColorRow` 拖动取色后父组件重渲染不得把受控 input 拨回原色、blur 提交选中色而非原色、未拖动开关则提交原色（真 no-op）；`ScrubInput` 拖动中父组件重渲染不得重置实时值、pointerUp 提交拖动值而非滞后 prop、空闲时仍从 prop 同步（外部 revert/重选）**。
